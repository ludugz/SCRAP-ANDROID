...
package eplus.scrap.common.rss;
...
class RSSHandler extends org.xml.sax.helpers.DefaultHandler {
  ...
  private final Setter SET_PUBDATE = new ContentSetter() {
     ...
     * {@link #item} is set.
     */
    private final Setter SET_IS_ATTENTION = new ContentSetter() {
        @Override
        public void set(String isAttention) {
            final int is_attention = Integers.parseInteger(isAttention);
            if (item != null) {
                item.setIsAttention(is_attention);
            }
        }
    };
    private final Setter SET_IS_PICKUP = new ContentSetter() {
        @Override
        public void set(String isPickup) {
            final int is_pickup = Integers.parseInteger(isPickup);
            if (item != null) {
                item.setIsPickup(is_pickup);
            }
        }
    };
	/**
  ...
  RSSHandler(RSSConfig config) {
    ...
    setters.put("isAttention", SET_IS_ATTENTION);
    setters.put("isPickup",SET_IS_PICKUP);
  }
